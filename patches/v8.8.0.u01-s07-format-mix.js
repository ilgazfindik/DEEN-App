(()=>{
 const VERSION='8.8.0-U01-S07-FORMAT-MIX',BUILD='880s07mix1';
 function runtime(){
  if(window.__deenV880U01S07FormatMix)return;window.__deenV880U01S07FormatMix=true;
  const VERSION='8.8.0-U01-S07-FORMAT-MIX',BUILD='880s07mix1',TARGET='U01-S07',MAX_SHARE=.25;
  ...
  function baseQuestion(q){const m=String(q?.id||'').match(/^DEEN-U01-S07-(\d{3})$/);return !!m&&Number(m[1])<=14}
  ...
  function install(){
   ...
   lastAudit=audit();document.documentElement.dataset.deenU01S07FormatMix=lastAudit.pass?'ready':'check';
   try{document.dispatchEvent(new CustomEvent('deen:u01-s07-format-mix',{detail:{version:VERSION,build:BUILD,...lastAudit}}))}catch(_){}
  ...
  window.DEEN_U01_S07_FORMAT_MIX={version:VERSION,build:BUILD,target:TARGET,maxShare:MAX_SHARE,install,audit,check:()=>({version:VERSION,build:BUILD,attempts,applied,changedIds:[...changedIds],audit:audit()})};
 }
 const addition='<!-- DEEN v8.8.0 — U01 S07 Format Mix -->\n<script id="deen-v880-u01-s07-format-mix-runtime">('+runtime.toString()+')();'+'</'+'script>';
 window.DEEN_PATCH_V880=function(html){let out=String(html);if(out.includes('deen-v880-u01-s07-format-mix-runtime'))return{html:out,version:VERSION,applied:0};out+='\n'+addition+'\n';return{html:out,version:VERSION,applied:1}};
})();
