(()=>{
 const VERSION='8.8.2-U01-S05-FORMAT-MIX',BUILD='882s05mix1';
 function runtime(){
  if(window.__deenV882U01S05FormatMix)return;window.__deenV882U01S05FormatMix=true;
  const VERSION='8.8.2-U01-S05-FORMAT-MIX',BUILD='882s05mix1',TARGET='U01-S05',MAX_SHARE=.25;
  ...
  function baseQuestion(q){const m=String(q?.id||'').match(/^DEEN-U01-S05-(\d{3})$/);return !!m&&Number(m[1])<=16}
  ...
  function install(){
   ...
   lastAudit=audit();document.documentElement.dataset.deenU01S05FormatMix=lastAudit.pass?'ready':'check';
   try{document.dispatchEvent(new CustomEvent('deen:u01-s05-format-mix',{detail:{version:VERSION,build:BUILD,...lastAudit}}))}catch(_){}
  ...
  window.DEEN_U01_S05_FORMAT_MIX={version:VERSION,build:BUILD,target:TARGET,maxShare:MAX_SHARE,install,audit,check:()=>({version:VERSION,build:BUILD,attempts,applied,changedIds:[...changedIds],audit:audit()})};
 }
 const addition='<!-- DEEN v8.8.2 — U01 S05 Format Mix -->\n<script id="deen-v882-u01-s05-format-mix-runtime">('+runtime.toString()+')();'+'</'+'script>';
 window.DEEN_PATCH_V882=function(html){let out=String(html);if(out.includes('deen-v882-u01-s05-format-mix-runtime'))return{html:out,version:VERSION,applied:0};out+='\n'+addition+'\n';return{html:out,version:VERSION,applied:1}};
})();
