(()=>{
 const VERSION='8.7.9-U01-S06-FORMAT-MIX',BUILD='879s06mix1';
 function runtime(){
  if(window.__deenV879U01S06FormatMix)return;window.__deenV879U01S06FormatMix=true;
  const VERSION='8.7.9-U01-S06-FORMAT-MIX',BUILD='879s06mix1',TARGET='U01-S06',MAX_SHARE=.25;
  ...
  function baseQuestion(q){const m=String(q?.id||'').match(/^DEEN-U01-S06-(\d{3})$/);return !!m&&Number(m[1])<=12}
  ...
  function install(){
   ...
   lastAudit=audit();document.documentElement.dataset.deenU01S06FormatMix=lastAudit.pass?'ready':'check';
   try{document.dispatchEvent(new CustomEvent('deen:u01-s06-format-mix',{detail:{version:VERSION,build:BUILD,...lastAudit}}))}catch(_){}
  ...
  window.DEEN_U01_S06_FORMAT_MIX={version:VERSION,build:BUILD,target:TARGET,maxShare:MAX_SHARE,install,audit,check:()=>({version:VERSION,build:BUILD,attempts,applied,changedIds:[...changedIds],audit:audit()})};
 }
 const addition='<!-- DEEN v8.7.9 — U01 S06 Format Mix -->\n<script id="deen-v879-u01-s06-format-mix-runtime">('+runtime.toString()+')();'+'</'+'script>';
 window.DEEN_PATCH_V879=function(html){let out=String(html);if(out.includes('deen-v879-u01-s06-format-mix-runtime'))return{html:out,version:VERSION,applied:0};out+='\n'+addition+'\n';return{html:out,version:VERSION,applied:1}};
})();
