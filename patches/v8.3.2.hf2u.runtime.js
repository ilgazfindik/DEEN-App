(()=>{
 ...
 function ensureCanvas(m,g){
   let c=$('.v832u-canvas',m);if(!c){c=document.createElement('div');c.className='v832u-canvas';c.innerHTML='<div class="v832u-avatar-slot"></div><span class="v832u-outfit" aria-hidden="true"></span><span class="v832u-status">GERÇEK KOMBİN</span>';m.appendChild(c)}c.dataset.gender=g;return c
 }
 ...
 function clean(r,c,g,id){
   const s=$('.v832u-status',c);if(s)s.textContent=(g==='male'?'ERKEK':'KADIN')+' · '+id;
   const f=$('.v831-footcopy',r);if(f)f.innerHTML='<b>Gerçek kıyafet + avatar uyumu</b><small>Karakter ölçeği ve kafa/yüz anchor sistemi test ediliyor. Kaydetme son kontrolden sonra açılacak.</small>';
 }
 function tick(){
   const r=root();if(!r?.classList?.contains('open'))return;const m=mannequin();if(!m)return;const g=gender(),id=activeId();if(!id)return;
   const c=ensureCanvas(m,g);syncOutfit(c,id);syncAvatar(c,g,id);clean(r,c,g,id);document.documentElement.dataset.deenHF2U='ready'
 }
 ...
 window.DEEN_HF2U={version:VERSION,refresh:tick,check:()=>({version:VERSION,gender:gender(),active:activeId(),outfit:!!$('.v832u-outfit',mannequin()),selectedAvatar:!!$('.v832u-selected-stack',mannequin()),saveLocked:true})};
